...
from transform.utils import UNUSED_CODE, randomColor, setPlace, splitArgs, sub
...
def convertLabel(name: str, argsMapping: Mapping[str, str]) -> str:
    ...
    return f"{name} = ttk.Label({form},{args}, bootstyle='{randomColor()}')"
...
def convertButton(name: str, argsMapping: Mapping[str, str]) -> str:
    ...
    return f"{name} = ttk.Button({form},{args}, bootstyle='{randomColor()}')"
...
def trans(content: str, unused) -> str:
    for each in unused:
...
def tk2ttkbootstrap(file: str | Path | None):
    ...
    with open(file, "r+", encoding="utf-8") as rf, open(
        "./new3.py", "x", encoding="utf-8"
    ) as wf:
        content = rf.read()
        content = trans(content, unused)
        wf.write("import ttkbootstrap as ttk\n")
        wf.write("from transform.utils import centerDlg")

        wf.write(content)

    os.system("black new3.py")


if __name__ == "__main__":
    file = "./init/tk2ttk.py"
    tk2ttkbootstrap(file)
